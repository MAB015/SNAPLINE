...
| Worktrees de `agent-*` arrancando desde un `main` desactualizado (pasó dos veces el 2026-09-22, y una vez más el 2026-09-23 con tres bloqueos previos consecutivos antes del intento que funcionó) | Sin mitigación de proceso todavía. El 2026-09-23 el especialista resolvió solo dentro de su worktree con `git merge --ff-only main` (su `HEAD` ya era ancestro de `main`, working tree limpio) en vez de reconstruir el trabajo; Product Manager verificó con `git merge-base --is-ancestor` antes de aceptar el resultado. Sigue pendiente revisar cómo CTO/orquestador crean los worktrees nuevos |
